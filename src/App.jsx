  ...
  return (
    <span style={{ position: "relative", display: "inline-block" }}>
      <span
        ...
        style={{
          cursor: "help", color: "#00e5ff", fontSize: 11, fontWeight: 700,
...
function PlanetViz({ radius }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 16, justifyContent: "center", padding: "20px 0", flexWrap: "wrap" }}>
      {PLANET_SIZES.filter(p => p.radius <= radius * 1.4 + 1).slice(0, 3).map(p => (
        <div key={p.name} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <div style={{ width: earthPx * p.radius * 0.6, height: earthPx * p.radius * 0.6, borderRadius: "50%", background: p.color, opacity: 0.4, minWidth: 6, minHeight: 6 }} />
        ...
        <div style={{ position: "relative" }}>
          ...
          {radius > 8 && (
            <div style={{
              ...
              borderRadius: "50%", border: `2px solid ${ringColor}50`, pointerEvents: "none",
            }} />
          )}
        </div>
        <span style={{ fontSize: 11, color: "#00e5ff", fontWeight: 700 }}>{radius.toFixed(2)} R⊕</span>
      </div>
    </div>
  );
}
...
function HistoryPanel({ history }) {
  ...
  return (
    <div style={{ maxHeight: 380, overflowY: "auto" }}>
      {history.map((h, i) => (
        <div key={i} style={{ padding: "10px 14px", marginBottom: 8, background: "rgba(13,26,48,0.6)", border: "1px solid #1a2240", borderRadius: 8, fontSize: 12 }}>
          ...
          {h.task === "classification" ? (
            <span style={{ color: h.result.prediction === "CONFIRMED" ? "#10b981" : "#ef4444", fontWeight: 700 }}>
  ...
  return (
    <div style={{ animation: "fadeIn 0.5s ease" }}>
      <div style={{
        ...
        <div style={{ fontSize: 13, color: "#64748b", marginBottom: 8, letterSpacing: "0.1em", textTransform: "uppercase" }}>Prediction Result</div>
        <div style={{ fontSize: 32, fontWeight: 900, color: confirmed ? "#10b981" : "#ef4444", letterSpacing: "-0.02em" }}>
          {result.prediction}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[
          ...
          <div key={m.label} style={{ padding: "12px 16px", background: "rgba(13,26,48,0.8)", border: "1px solid #1a2240", borderRadius: 8, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#334155", marginBottom: 4 }}>{m.label}</div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "#00e5ff", fontFamily: "'JetBrains Mono', monospace" }}>{m.value}</div>
            <div style={{ fontSize: 9, color: "#1e3a5f", marginTop: 2 }}>{m.note}</div>
          </div>
        ))}
      </div>
      <div style={{ padding: 16, background: "rgba(13,26,48,0.5)", border: "1px solid #1a2240", borderRadius: 8 }}>
        <div style={{ fontSize: 11, color: "#334155", marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.08em" }}>Feature Importance</div>
        <ImportanceChart data={result.feature_importance} />
      </div>
      {result.latency_ms && (
        <div style={{ textAlign: "right", fontSize: 10, color: "#1e3a5f", marginTop: 10 }}>Inference: {result.latency_ms}ms</div>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ animation: "fadeIn 0.5s ease" }}>
      <div style={{
        ...
        <div style={{ fontSize: 13, color: "#64748b", marginBottom: 8, letterSpacing: "0.1em", textTransform: "uppercase" }}>Predicted Radius</div>
        <div style={{ fontSize: 40, fontWeight: 900, color: "#f59e0b", letterSpacing: "-0.03em" }}>
          {r.toFixed(2)} R⊕
        </div>
        <div style={{ display: "inline-block", marginTop: 8, padding: "4px 14px", borderRadius: 20, background: "rgba(124,58,237,0.2)", border: "1px solid rgba(124,58,237,0.4)", fontSize: 12, color: "#a78bfa", fontWeight: 700 }}>
      ...
      <div style={{ padding: 16, background: "rgba(4,6,15,0.6)", border: "1px solid #1a2240", borderRadius: 10, marginBottom: 20 }}>
        <div style={{ fontSize: 11, color: "#334155", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.08em" }}>Size Comparison</div>
        <PlanetViz radius={r} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[
          ...
          <div key={m.label} style={{ padding: "12px 16px", background: "rgba(13,26,48,0.8)", border: "1px solid #1a2240", borderRadius: 8, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#334155", marginBottom: 4 }}>{m.label}</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#a78bfa", fontFamily: "'JetBrains Mono', monospace" }}>{m.value}</div>
            <div style={{ fontSize: 9, color: "#1e3a5f", marginTop: 2 }}>{m.note}</div>
          </div>
        ))}
      </div>
...
function validateInputs(values, features) {
  ...
  features.forEach(f => {
    ...
    if (num < f.min - Math.abs(f.min) * 0.5 || num > f.max * 5) {
      errors[f.key] = `Unusual value (typical: ${f.min}–${f.max})`;
    }
  });
  return errors;
}
...
export default function App() {
  ...
  const features = activeTask === "classification" ? CLF_FEATURES : REG_FEATURES;
  ...
  // ── Only updates values, does NOT clear them ──
  const switchTask = (newTask) => {
    setActiveTask(newTask);
    setResult(null);
    setApiError(null);
    // values and errors are intentionally preserved
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const endpoint = activeTask === "classification"
  ...
  return (
    <>
      ...
      <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", paddingBottom: 60 }}>
        ...
        {/* ── Header ── */}
        <header style={{
          padding: "20px 40px 16px", borderBottom: "1px solid #0d1630",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: "#334155", letterSpacing: "0.12em", textTransform: "uppercase", marginTop: -2 }}>
        ...
        {tab === "history" ? (
          <div style={{ maxWidth: 720, margin: "40px auto", padding: "0 24px" }}>
            <div style={{ fontSize: 22, fontWeight: 800, marginBottom: 20, color: "#94a3b8" }}>Prediction History</div>
            <HistoryPanel history={history} />
          </div>

        ) : (
          /* ── Predict Tab ── */
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px", display: "grid", gridTemplateColumns: "1fr 420px", gap: 24 }}>

            {/* LEFT: Form */}
            <div>

              {/* Task selector — calls switchTask, NOT clearAll */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 28 }}>
                {[
                  { id: "classification", label: "Task A — Classification", sub: "CONFIRMED vs FALSE POSITIVE", icon: "◉", color: "#00e5ff" },
                  { id: "regression",     label: "Task B — Regression",     sub: "Predict planetary radius",    icon: "◎", color: "#a78bfa" },
                ].map(t => (
                  <button key={t.id} onClick={() => switchTask(t.id)} style={{
                    padding: "16px 20px", borderRadius: 10, cursor: "pointer", textAlign: "left",
                    background: activeTask === t.id
                      ? `linear-gradient(135deg, rgba(${t.id === "classification" ? "0,229,255" : "124,58,237"},0.08), transparent)`
                      : "rgba(8,13,30,0.8)",
                    border: `2px solid ${activeTask === t.id ? (t.id === "classification" ? "#00e5ff" : "#7c3aed") + "60" : "#1a2240"}`,
                    color: "#e2e8f0", fontFamily: "'Space Grotesk', sans-serif", transition: "all 0.2s",
                  }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
                <span style={{ fontSize: 11, color: "#334155", textTransform: "uppercase", letterSpacing: "0.08em" }}>Quick fill:</span>
                <button onClick={() => fillSample(SAMPLE_CONFIRMED)} style={{ padding: "5px 12px", borderRadius: 5, fontSize: 11, cursor: "pointer", background: "rgba(16,185,129,0.1)", border: "1px solid #10b98130", color: "#10b981", fontFamily: "'Space Grotesk', sans-serif" }}>✓ Confirmed example</button>
                <button onClick={() => fillSample(SAMPLE_FP)} style={{ padding: "5px 12px", borderRadius: 5, fontSize: 11, cursor: "pointer", background: "rgba(239,68,68,0.1)", border: "1px solid #ef444430", color: "#ef4444", fontFamily: "'Space Grotesk', sans-serif" }}>✗ False positive example</button>
                <button onClick={clearAll} style={{ padding: "5px 12px", borderRadius: 5, fontSize: 11, cursor: "pointer", background: "transparent", border: "1px solid #1a2240", color: "#475569", fontFamily: "'Space Grotesk', sans-serif" }}>Clear</button>
              </div>
              ...
              {[
                {
                  ...
                  keys: ["koi_period", "koi_duration", "koi_depth", "koi_impact", "koi_model_snr", "koi_num_transits",
                         ...(activeTask === "classification" ? ["koi_ror"] : [])],
                },
                {
                ...
                },
                {
                  ...
                  keys: ["teff_err1", "teff_err2", "logg_err1", "logg_err2", "feh_err1", "feh_err2", "mass_err1", "mass_err2", "radius_err1", "radius_err2"],
                },
              ].map(group => {
                const groupFeats = features.filter(f => group.keys.includes(f.key));
                if (!groupFeats.length) return null;
                return (
                  <div key={group.title} style={{ marginBottom: 24, padding: 20, borderRadius: 12, background: "rgba(8,13,30,0.7)", border: "1px solid #1a2240" }}>
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em" }}>{group.title}</div>
              ...
              <button onClick={handleSubmit} disabled={loading} style={{
                ...
                background: loading ? "#1a2240" : activeTask === "classification"
                ...
                letterSpacing: "0.04em",
                boxShadow: loading ? "none" : activeTask === "classification"
                  ? "0 4px 24px rgba(0,229,255,0.2)"
                  : "0 4px 24px rgba(124,58,237,0.3)",
                transition: "all 0.2s",
              }}>
                {loading ? "⟳  Running inference..." : activeTask === "classification"
            ...
            <div style={{ position: "sticky", top: 90, alignSelf: "start" }}>
              <div style={{ padding: 24, borderRadius: 12, background: "rgba(8,13,30,0.9)", border: "1px solid #1a2240", minHeight: 300 }}>
                <div style={{ fontSize: 11, color: "#334155", marginBottom: 16, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                  Inference Output
                </div>

                {apiError && !loading && (
                  <div style={{ padding: 16, borderRadius: 8, background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.3)", color: "#fca5a5", fontSize: 13, animation: "fadeIn 0.3s ease" }}>
                ...
                {loading && (
                  <div style={{ textAlign: "center", padding: 40 }}>
                    <div style={{
                      ...
                      borderTopColor: activeTask === "classification" ? "#00e5ff" : "#7c3aed",
                ...
                {!loading && !result && !apiError && (
                  <div style={{ textAlign: "center", padding: 60, color: "#1e3a5f" }}>
                    ...
                    <div style={{ fontSize: 11, marginTop: 6, color: "#1a2240" }}>Or use Quick Fill to load a sample</div>
                  </div>
                )}
              </div>

              {/* Dataset stats */}
              <div style={{ marginTop: 16, padding: 16, borderRadius: 10, background: "rgba(8,13,30,0.6)", border: "1px solid #0d1630" }}>
                <div style={{ fontSize: 10, color: "#1e3a5f", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>Dataset Overview</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {[
                    ...
                    <div key={s.label} style={{ padding: "8px 10px", background: "rgba(4,6,15,0.8)", borderRadius: 6, border: "1px solid #0d1630", textAlign: "center" }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: s.color || "#475569", fontFamily: "'JetBrains Mono', monospace" }}>{s.value}</div>
                      <div style={{ fontSize: 9, color: "#1e3a5f", marginTop: 2 }}>{s.label}</div>
                    </div>
                  ))}
                </div>
                <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <div style={{ padding: "8px 10px", background: "rgba(4,6,15,0.8)", borderRadius: 6, border: "1px solid #0d1630", textAlign: "center" }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: "#00e5ff", fontFamily: "'JetBrains Mono', monospace" }}>0.8977</div>
                    <div style={{ fontSize: 9, color: "#1e3a5f", marginTop: 1 }}>F1-Score (Task A)</div>
                  </div>
                  <div style={{ padding: "8px 10px", background: "rgba(4,6,15,0.8)", borderRadius: 6, border: "1px solid #0d1630", textAlign: "center" }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: "#a78bfa", fontFamily: "'JetBrains Mono', monospace" }}>0.9810</div>
